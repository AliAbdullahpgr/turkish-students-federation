...
export default function NavigationListPage() {
  ...
  useEffect(() => {
    fetch("/api/admin/navigation").then((r) => r.json()).then(setItems).finally(() => setLoading(false));
  }, []);
                  ...
                  <tbody key={item.id}>
                    <tr className="border-b border-border-custom hover:bg-surface/50 bg-surface/30">
                      ...
                      <td className="p-4 text-sm">
                        <button onClick={() => toggleVisibility(item.id, item.isVisible)}>{item.isVisible ? "✅" : "❌"}</button>
                      </td>
                      <td className="p-4 text-right">
                        <Link href={`/admin/navigation/${item.id}/edit`} className="p-2 text-text-secondary hover:text-primary">Düzenle</Link>
                        <button onClick={() => handleDelete(item.id)} className="p-2 text-text-secondary hover:text-turkish-red ml-1">Sil</button>
                      </td>
                    </tr>
                    {children.map((child) => (
                      <tr key={child.id} className="border-b border-border-custom hover:bg-surface/50">
                        ...
                        <td className="p-4 text-sm">
                          <button onClick={() => toggleVisibility(child.id, child.isVisible)}>{child.isVisible ? "✅" : "❌"}</button>
                        </td>
                        <td className="p-4 text-right">
                          <Link href={`/admin/navigation/${child.id}/edit`} className="p-2 text-text-secondary hover:text-primary">Düzenle</Link>
                          <button onClick={() => handleDelete(child.id)} className="p-2 text-text-secondary hover:text-turkish-red ml-1">Sil</button>
                        </td>
                      </tr>
                    ))}
                  </tbody>
